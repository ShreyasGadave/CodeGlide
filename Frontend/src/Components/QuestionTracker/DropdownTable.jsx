...
export default function DropdownTable({ topics, sheetId }) {
  ...
  const markAsSolved = async (questionId) => {
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL}/api/sheets/question/mark-solved`,
        {
          sheetId,
          questionId,
        },
  ...
  const fetchNoteForQuestion = async (question) => {
    ...
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/notes/${question.noteId}`,
      );
              ...
              <Table>
                <TableBody>
                  {topicData.questions.map((q) => {
                    const isSolved =
                      q.status === "Completed" || solvedQuestions[q.questionId];
                    return (
                      <TableRow key={q.questionId} className="hover:bg-gray-50">
                        <TableCell>
                          <button
                            onClick={() => markAsSolved(q.questionId)}
                            className="focus:outline-none"
                          >
                            {isSolved ? (
                              <FaCheckCircle className="text-green-500 w-5 h-5" />
                            ) : (
                              <FaRegCircle className="text-green-500 w-5 h-5" />
                            )}
                          </button>
                        </TableCell>
                        ...
                        <TableCell>
                          ...
                          >
                            <FiClipboard
                              className={`w-5 h-5 ${q.noteId ? "text-yellow-500" : "text-gray-500"}`}
                            />
                          </Button>
                        </TableCell>
                      </TableRow>
