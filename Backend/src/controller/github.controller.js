...
import GithubDatas from "../model/github.model.js";
...
const GithubData = async (req, res) => {
  try {
    ...
    // ⏱️ Check cached data
    const existingData = await GithubDatas.findOne({ userId });
    ...
    const userProfilePromise = axios.get(
    ...
    const [repos, prData, issuesData, starsData, graphqlData] =
      await Promise.all([
        reposPromise,
        prPromise,
        issuesPromise,
    ...
    // 🔥 Heatmap
    const heatmap =
      graphqlData.user.contributionsCollection.contributionCalendar.weeks.flatMap(
    ...
    const totalCommits =
      graphqlData.user.contributionsCollection.totalCommitContributions;
    ...
    // 💾 Save to DB
    await GithubDatas.findOneAndUpdate(
      { userId },
